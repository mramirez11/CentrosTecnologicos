  ...
  <v-container>
    <v-form class="pl-6 pr-6" ref="form" v-model="valid">
      ...
      <v-layout justify-center class="pb-5 pt-3 blue--text">
        <h5 class="font-weight text-h5">Titulo</h5>
      </v-layout>


      <!-- Panel de  -->
      <v-layout v-for="(item, index) in " :key="index">
          ...
          <v-card color="#F2F2F2" class="mt-6 pl-8 pr-8">
            <v-layout justify-end>
              <v-layout>
                ...
                <v-btn icon color="red" @click="delete (index)">
            ...
            <v-row>
              <!--  TextField -->
              <v-col>
                <v-text-field v-model="item." label="" :rules="inputRules" required></v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <!-- Rut TextField -->
