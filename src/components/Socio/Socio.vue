<template>
  <v-container>
    <v-form class="pl-6 pr-6" ref="form" v-model="valid">
      ...
      <!-- Panel de  -->
      <v-layout v-for="(item, index) in socio" :key="index">
...
export default {
  data() {
    return {
      vinculaciones: ["Utalca", "Nacional", "Internacional"],
      instituciones: ["Facultad", "Centro Tecnológico", "Unidad Académica"],
      sectores: ["A definir"],
      colaboraciones:["Proyecto I+D", "Levantamiento y difusión de información", "Actividades de transferencia tecnológica e innovación", "Actividades de docencia", "Publicaciones", "Otros"],
  ...
  computed: {
    ...mapState(["socio", "persona"])
  },
  methods: {
    ...mapMutations(["nuevoSocio", "deleteSocio", "goTo"])
  }
};
</script>
